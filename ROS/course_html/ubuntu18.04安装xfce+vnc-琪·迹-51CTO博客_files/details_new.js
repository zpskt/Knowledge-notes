(function(){
	var clientWidth = (document.body.clientWidth - 1200) / 2 - 100
	if(clientWidth > 0) {
		var fixedRight =  clientWidth.toFixed(2)
		$('.group_img_box').css('right', fixedRight+'px')
	} else {
		$('.group_img_box').css('right', '0px')
	}
	...
	lowest();
	var tableTop = $('.table-contents').offset().top
	var commentTop = $('.author-module').offset().top
    ...
    function lowest(){
    	if($(window).scrollTop()-30+$(window).height() > $('.more-list').offset().top){ 
			$('.function-bar').removeClass('fix-function')
	    }else{
			$('.function-bar').addClass('fix-function')
	    }
	}
	// 评论吸底
	function fixFun() {
	...
	function fixedTable() {
		if($(window).scrollTop()>tableTop){
			$('.table-contents').addClass('fix-table')
		}else{
			$('.table-contents').removeClass('fix-table')
		}
	}
	...
	$.each(ts,function(i,e){
		$(this).attr('id','h'+i);
		var href = window.location.href.split('#')[0] +'#h'+i
		var Lv = 1
		// var item = $('<li><a href="'+href+'">'+$(e).text()+'</a></li>').click(function(){
		//   $(window).scrollTop($(e).offset().top)
		//   $(this).addClass('on').siblings().removeClass('on')
		// })
		var item = $('<li><a href="'+href+'">'+$(e).text()+'</a></li>')
		list.append(item)
		...
		item.addClass('lv'+Lv)
		if(e.tagName=='H1' == 1) {
			item.children('a').html('第'+h1_num+'章&nbsp;&nbsp;'+$(e).text())
			h1_num++
		}
	})
	// 滑块
